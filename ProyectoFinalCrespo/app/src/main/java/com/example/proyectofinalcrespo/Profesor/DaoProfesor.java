...
public class DaoProfesor {
    ...
    public ProfesorModelo buscar(String dni) {
        ProfesorModelo profesorModelo = new ProfesorModelo();


        Cursor c = db.rawQuery("SELECT * from Profesor where dni =" + dni, null);
        if (c.moveToFirst()) {
            do {
                profesorModelo.setDni(c.getInt(0));
                profesorModelo.setNombre(c.getString(1));
                profesorModelo.setApellido(c.getString(2));
                profesorModelo.setDomicilio(c.getString(3));
                profesorModelo.setTelefono(c.getString(4));
